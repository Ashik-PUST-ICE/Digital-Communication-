...
# Take input from the user for fc, fm, and duty cycle
fc = float(input("Enter the carrier frequency (fc): "))
fm = float(input("Enter the modulation frequency (fm): "))
...
on_samp = int(np.ceil(period_samp * duty))
pam = np.zeros(len(n))
for i in ind:
    pam[i:i+on_samp] = m[i]
